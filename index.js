...
io.on("connection", socket => {
	...
	if (user) {
		if ("roomId" in user) {
			...
			let fellowPlayers = allPlayers.filter(
	...
	socket.on("endOfGame", () => {
		...
		if (winner.length > 1) {
			io.sockets.in(user.id).emit("endResult", "It's a tie!");
		} else {
			const loser = fellowPlayers.find(player => player.id != winner.id);
			io.sockets
				.in(winner.id)
				.emit("endResult", "Congratulations! You have won :~)");
			io.sockets
				.in(loser.id)
				.emit("endResult", "Unfortunately. You have lost :~(");
		}
	});
